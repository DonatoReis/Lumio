...
-- ============================================================================
-- CRIAÇÃO DA TABELA ADMINS (ADICIONADO PARA CORRIGIR ERRO NA MIGRAÇÃO)
-- ============================================================================
...
-- Configurar RLS para a tabela admins
ALTER TABLE admins ENABLE ROW LEVEL SECURITY;

-- Apenas administradores podem visualizar a tabela admins
CREATE POLICY "Admins só são visíveis para admins" ON admins
    FOR SELECT USING (
        user_id = auth.uid() OR 
        EXISTS (SELECT 1 FROM admins WHERE user_id = auth.uid())
    );

-- Apenas super-admins podem modificar a tabela admins (via service role)
CREATE POLICY "Admins só podem ser alterados pelo sistema" ON admins
...
-- Criar trigger para notificar mudanças nas features do usu
